...
date: "Master G2M, 5-7 septembre 2022"
author: 
 - name: Hugues Pecout
...
nocite: |
  @*
link-citations: true
licence: "![licensebuttons by-sa](https://licensebuttons.net/l/by-sa/4.0/88x31.png)"  
...
Pour éviter des problèmes d'encodage des scripts ou le chargement incessant de données à chaque nouveau démarrage de RStudio, **il est nécessaire de modifier certains paramètres globaux**. Pour cela, cliquez sur "*tools*" puis sur "*Global Options*" pour ouvrir la fenêtre des paramètre généraux de RStudio. 
...
name <-letters[1:10]
latitude <- c(48.84905, 48.85217, 48.83349, 48.86528, 48.86409, 
              48.85176, 48.85207, 48.88334, 48.85758, 48.87391)
longitude <- c(2.331454, 2.347332, 2.318518, 2.371150, 2.356515, 
               2.299024, 2.358712, 2.333936, 2.352751, 2.343179)

mes_donnees <- data.frame(ID, name, latitude, longitude)

library(sf)
mes_donnees_SF <- st_as_sf(mes_donnees, coords = c("longitude", "latitude"), 
                           crs = 4326)
mes_donnees_SF
...
Il y a plusieurs façons d'obtenir de l'aide avec R. **Vous pouvez utiliser l'onglet d'aide proposé par RStudio** et accéder à la documentation d'une fonction :
...
# Import d'un fichier 
# not run
Mon_Objet <- read.table("/data/user/documents/mes.donnes.csv", 
...
Pour sélectionner des lignes et des colonnes dans un `data.frame` nous pouvons utiliser les crochets :   
`df[criteres_de_selection_des_lignes,  criteres_de_selection_des_colonnes]`
...
Ici nous sélectionnons les lignes de `mtcars` où la variable "cyl" est égale à 4 et la variable "wt" est supérieure à 3 et nous ne gardons que les 7 premières colonnes. 

Il est aussi possible d'utiliser la fonction `subset()` ou le package `dplyr`. 
...
**Le regroupement de lignes est une opération classique en gestion de base de données**. 

Nous pouvons utiliser la fonction `aggregate()` : 



```{r class.source="bg-info", class.output="bg-warning", warning=FALSE, message=FALSE}

# moyenne des valeurs de la variable mpg en fonction des valeurs de la variables "cyl"
aggregate(x = mtcars$mpg, by = list(cyl = mtcars$cyl), FUN = mean)

```

Il est aussi possible d'utiliser les packages `dplyr` ou `data.table`.
...
# Nom des variables à utiliser : "P16_POP" et "CATAEU2010"
...
emails_list <- c("hugues.pecout@cnrs.fr", 
                 "hugues@gmail", 
                 "blabla.com", 
                 "hugues.pecout@gis-vist.fr",
                 "timothée.giraud@cmrs.fr")
...
**Plusieurs éléments peuvent être superposés**, par exemple :                   
...
tab_occup <- table(hdv2003$occup)
tab_occup
# trier les modalités en fonction de leur fréquence
tab_occup <- sort(tab_occup, decreasing = TRUE)
tab_occup

# Effectifs relatifs (%)
prop.table(tab_occup)
...
cols <- hcl.colors(n = 9, palette = "Dynamic")

# Diagramme circulaire (Pas optimal...)
pie(tab_occup, col = cols)

# Histogramme
barplot(tab_occup, col = cols, las=2, cex.names = 0.6)
```
...
ma_discretisation <- mf_get_breaks(x = mtcars$hp, 
                                   breaks = "quantile", 
                                   nbreaks = 6)
ma_discretisation
...
mtcars$hp_classe <- cut(x = mtcars$hp, 
...
levels(mtcars$hp_classe)<-c("A","B","C","D","E")
mtcars$hp_classe
...
<p align="center"> Source : https://riatelab.github.io/mapsf</p>
...
mtq_data <- read.csv("data/INSEE_COM_972.csv", header=TRUE, sep=",")
...
```{r class.source="bg-info", class.output="bg-warning", warning=FALSE, message=FALSE, eval=TRUE, fig.width=5.5}
...
5) **Discrétisez la variable** ***TX_CS1_2016***.   
  Quelle méthode choisissez-vous ? Pourquoi ?     
<br/>
...
# Pourcentage en ligne d'un tableau croisé table(), avec rprop() du package 'questionnr'
...
ggplot(tab, aes(x = age, y = Freq, fill = factor(sport))) +
  ...
  ylab("Pourcentage") +
  scale_x_discrete(breaks = seq(15,100,5), labels = seq(15,100,5)) +
...
```{r class.source="bg-info", class.output="bg-warning", warning=FALSE, message=FALSE}
plot(x = rp2012$dipl_sup, y = rp2012$cadres)
```
...
plot(my_reg, which = c(1), col=c("blue"))
plot(my_reg, which = c(3), col=c("blue"))
```
